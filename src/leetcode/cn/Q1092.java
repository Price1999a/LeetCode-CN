...
public class Q1092 {
    ...
    public static void main(String[] a) {
        System.out.println(new Q1092().shortestCommonSupersequence("abac", "cab"));
    }
    ...
    public String shortestCommonSupersequence(String str1, String str2) {
        ...
        for (int i = 0; i <= n; i++) {
            dp[i][m] = str1.substring(i);
        }
        for (int i = 0; i <= m; i++) {
            dp[n][i] = str2.substring(i);
        }
        for (int i = n - 1; i >= 0; i--)
            for (int j = m - 1; j >= 0; j--) {
                if (str1.charAt(i) == str2.charAt(j))
                    ...
                    dp[i][j] = (dp[i + 1][j].length() < dp[i][j + 1].length()) ? (str1.charAt(i) + dp[i + 1][j]) : (str2.charAt(j) + dp[i][j + 1]);
            }
        return dp[0][0];
    }
